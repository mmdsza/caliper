...
Monaco-based grader editor for Caliper — the authoring surface for `@grader`
...
- A sample `@grader` (`lib/sample-grader.ts`) — the "format gate + correctness"
  pattern from `docs/03-graders-and-rewards.md`, adapted to the Caliper SDK
  shape (`from caliper_sdk import grader, Rollout, GraderResult`).
- Drafts persist to `localStorage` under the key `caliper.editor.draft`; on
...
- No live SDK type-checking — the editor doesn't know about `caliper_sdk`'s
