...
Selection is by environment:
    ...
    CALIPER_GRADER_EXECUTOR=in_process  # default
    CALIPER_GRADER_EXECUTOR=e2b
"""
...
class E2BExecutor:
    ...
    1. Build a wheel of the workspace ``caliper-sdk`` (cached across calls
    ...
    DRIVER_PATH = "/tmp/caliper_driver.py"  # noqa: S108 - sandbox tmpfs, not host
    SOURCE_PATH = "/tmp/caliper_grader.py"  # noqa: S108 - sandbox tmpfs, not host
    WHEEL_PATH = "/tmp/caliper_sdk.whl"  # noqa: S108 - sandbox tmpfs, not host
        ...
        if sandbox_factory is None:
            try:
                ...
                raise ImportError(
                    ...
                    "Install with: pip install 'caliper-server[e2b]'"
                ) from e
...
def build_from_env(env: dict[str, str] | None = None) -> GraderExecutor:
    """Pick an executor based on ``CALIPER_GRADER_EXECUTOR``.

    Defaults to :class:`InProcessExecutor` so ``uv run pytest`` works
    without any external setup.
    """
    chosen = (env or os.environ).get("CALIPER_GRADER_EXECUTOR", "in_process").strip().lower()
    if chosen in {"", "in_process", "inprocess", "local"}:
        return InProcessExecutor()
    if chosen == "e2b":
        return E2BExecutor()
    raise ValueError(
        f"unknown CALIPER_GRADER_EXECUTOR={chosen!r}; expected 'in_process' or 'e2b'"
    )
...
def _build_sdk_wheel() -> Path:
    """Build a wheel of the workspace `caliper-sdk` package.
    ...
    if not (sdk_dir / "pyproject.toml").exists():
        raise SandboxError(
            f"could not locate caliper-sdk source at {sdk_dir} — "
            "E2BExecutor must run inside a workspace checkout"
        )

    out_dir = Path(tempfile.gettempdir()) / "caliper-sandbox-wheels"
    ...
    if proc.returncode != 0:
        raise SandboxError(f"failed to build caliper-sdk wheel: {proc.stderr or proc.stdout}")

    wheels = sorted(out_dir.glob("caliper_sdk-*.whl"), key=lambda p: p.stat().st_mtime)
    if not wheels:
