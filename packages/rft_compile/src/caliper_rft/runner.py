...
This module is the SDK-side glue that turns an Caliper grader spec into a
...
* **Pluggable client.** `RFTClient` is a Protocol. The default real impl is
  `caliper_rft.clients.OpenAIRFTClient`; a `FakeRFTClient` (in
  `caliper_rft.testing`) is used for unit tests and any caller who wants
...
* **Pure compile + submit separation.** The runner accepts an `AnyGrader`
  (the typed spec from `caliper_rft.spec`), compiles it via
...
from caliper_rft.compile import compile_to_rft
from caliper_rft.spec import AnyGrader
